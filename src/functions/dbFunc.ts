...
export async function search_document(
  ...
  try {
    const skip = (page - 1) * 10;
    // Split the search terms into individual words
const custom = "\"thrones\" \"s02\" \"psa\" \"720p\""
    // Count filtered documents
...
export async function search_video(
  searchTerms: string,
  page: number
): Promise<{ filteredDocs: any[]; totalsize: number }> {
  try {
...
export async function search_audio(
  searchTerms: string,
  page: number
): Promise<{ filteredDocs: any[]; totalsize: number }> {
  try {
...
export async function search_document_file_id(data: any): Promise<any | null> {
  try {
    const filteredDocs = await db.DocumentCollection.findOne(
      { file_unique_id: data },
      { projection: { file_id: 1, file_name: 1, file_caption: 1, _id: 0 } }
    );
    return { filteredDocs };
  } catch (error: any) {
...
export async function search_video_file_id(data: any): Promise<any | null> {
  try {
    const filteredDocs = await db.VideoCollection.findOne(
      { file_unique_id: data },
      { projection: { file_id: 1, file_name: 1, file_caption: 1, _id: 0 } }
    );
    return { filteredDocs };
...
export async function search_audio_file_id(data: any): Promise<any | null> {
  try {
    const filteredDocs = await db.AudioCollection.findOne(
      { file_unique_id: data },
      { projection: { file_id: 1, file_name: 1, file_caption: 1, _id: 0 } }
    );
    return { filteredDocs };
...
export async function get_file_details(data: any): Promise<any | null> {
  try {
    ...
    const doc_result = await db.DocumentCollection.findOne(
      file_unique_id,
      updateDoc
    );
    const vid_result = await db.VideoCollection.findOne(
      file_unique_id,
      updateDoc
    );
    const aud_result = await db.AudioCollection.findOne(
      file_unique_id,
      updateDoc
    );
...
export async function is_user_banned(data: any): Promise<any | null> {
  try {
    ...
    const { is_banned } = await db.UserCollection.findOne(
      { user_id: user_id },
      { projection: { is_banned: 1, _id: 0 } }
    );
    return { is_banned };
  } catch (error: any) {
    console.log(error.message);
...
export async function terminate_user_files(data: any): Promise<any | null> {
  try {
    ...
    const doc_mod_result = await db.DocumentCollection.updateMany(
      user_id,
      updateDoc
    );
    const vid_mod_result = await db.VideoCollection.updateMany(
      user_id,
      updateDoc
    );
    const aud_mod_result = await db.AudioCollection.updateMany(
      user_id,
      updateDoc
    );
    const user_mod_result = await db.UserCollection.updateOne(
      user_id,
      updateDoc
    );
    return {
...
export async function terminate_user_files_reply(
  data: any
): Promise<any | null> {
  try {
    const file_unique_id = { file_unique_id: data };
...
export async function reinstate_user_files_reply(
  data: any
): Promise<any | null> {
  try {
    const file_unique_id = { file_unique_id: data };
...
export async function remove_file(data: any): Promise<any | null> {
  try {
    ...
    const doc_mod_result = await db.DocumentCollection.updateOne(
      file_unique_id,
      updateDoc
    );
    const vid_mod_result = await db.VideoCollection.updateOne(
      file_unique_id,
      updateDoc
    );
    const aud_mod_result = await db.AudioCollection.updateOne(
      file_unique_id,
      updateDoc
    );
